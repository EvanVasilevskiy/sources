	   ...
	   else
	   {
		   ...
		   for(int i = istart; i <= kolvo + istart + autosmesh && i < maxicandles; i++)
		   {
			  ...
			  if(ww <= 2) bit->Canvas->Pen->Width = ww;
			  {
				  ...
				  bit->Canvas->FillRect(Rect(xx, frame.y + frame.height - 20 - cndl.volume*zoomvol /*ZoomVolumes->Position/500000.0*/,
			  ...
			  if(dV->Checked)
			  {
				 if(klast[i].volume_bue >= klast[i].volume_sell)
				 {
					...
					bit->Canvas->MoveTo(t1, frame.y + frame.height - 20 - klast[i].volume_bue*zoomvol /*ZoomVolumes->Position/500000.0*/);
					bit->Canvas->LineTo(t1, frame.y + frame.height - 20);
					bit->Canvas->Pen->Color = RGB(250, 100, 100);
					bit->Canvas->MoveTo(t1, frame.y + frame.height - 20 - klast[i].volume_sell*zoomvol /*ZoomVolumes->Position/500000.0*/);
					bit->Canvas->LineTo(t1, frame.y + frame.height - 20);
				 }
				 else
				 {
			  ...
			  sttmin5 += sttmin;
			  sttmin = GetTickCount();
		   }            sttmin1, sttmin2, sttmin3, sttmin4, sttmin5;
			   sttmin1_1, sttmin1_2, sttmin1_3, sttmin1_4;
